...
class FCN(BaseNet):
    ...
    def _build_loss(self, logits, label):
        ...
        with tf.variable_scope('Losses'):
            loss_function.get_loss(self.loss['name'], logits, label, classes=self.classes, **self.loss)
            ...
            for key, value in self.classes.items():
                if value['show']:
                    ...
                    for name, args in self.train_metrics.items():
                        result = metrics_function.get_mertrics(name, logits, label,
                                                               key + '_' + CustomKeys.METRICS + '_' + name, **args)
